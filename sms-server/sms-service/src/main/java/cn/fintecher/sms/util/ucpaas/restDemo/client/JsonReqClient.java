 ...
 * @project rest_demo
 */
package cn.fintecher.sms.util.ucpaas.restDemo.client;
...
import cn.fintecher.sms.util.ucpaas.restDemo.DateUtil;
import cn.fintecher.sms.util.ucpaas.restDemo.EncryptUtil;
import cn.fintecher.sms.util.ucpaas.restDemo.models.AppBill;
import cn.fintecher.sms.util.ucpaas.restDemo.models.Callback;
import cn.fintecher.sms.util.ucpaas.restDemo.models.ClientBill;
import cn.fintecher.sms.util.ucpaas.restDemo.models.TemplateSMS;
import cn.fintecher.sms.util.ucpaas.restDemo.models.VoiceCode;
public class JsonReqClient extends AbsRestClient {
	 ...
	 */
	@Override
	public String voiceCode(String accountSid, String authToken, String appId,String to, String captchaCode,String displayNum, String playTimes) {
		// TODO Auto-generated method stub
		String result = "";
		DefaultHttpClient httpclient=getDefaultHttpClient();
		try {
			...
			//-----https:server.com/加密字符串/版本/Accounts/帐户ID/Calls/voiceCode?sig=signature
			// 构造请求URL内容
			String timestamp = DateUtil.dateToStr(new Date(),DateUtil.DATE_TIME_NO_SLASH);// 时间戳
			String signature = getSignature(accountSid,authToken,timestamp,encryptUtil);
			String url = getStringBuffer().append("/").append(version)
			...
			voiceCode.setCaptchaCode(captchaCode);
			voiceCode.setPlayTimes(playTimes);
			voiceCode.setTo(to);
			voiceCode.setDisplayNum(displayNum);
			Gson gson = new Gson();
